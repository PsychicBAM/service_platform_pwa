...
import { createPlanChangeRequest, listAdminServices } from "@/api/adminApi";
import {
  formatLimit,
  getHeroUpgradeCopy,
  getPublicPlan,
...
import type { BusinessAdminRead, CheckoutPlanId, SubscriptionPlan } from "@/types/api";
import { getApiErrorMessage } from "@/utils/errors";

export type CheckoutActionResult =
  | { outcome: "redirect"; message?: string }
  | { outcome: "manual_upgrade"; message: string }
...
type AdminPaymentsBillingPanelProps = {
  ...
  onStartCheckout: (plan: CheckoutPlanId) => Promise<CheckoutActionResult>;
};
...
export function AdminPaymentsBillingPanel({
  ...
  const [actionLoadingPlan, setActionLoadingPlan] = useState<PublicPlanId | null>(null);
  const [feedback, setFeedback] = useState<PlanActionFeedback | null>(null);
  ...
  const isBusy = actionLoadingPlan !== null || checkoutLoadingPlan !== null;
  ...
  async function submitPlanChangeRequest(
    ...
    await createPlanChangeRequest(businessId, { requested_plan: requestedPlan });
    setFeedback({
      tone: "success",
      message:
        kind === "upgrade"
          ? "Upgrade request sent to superadmin."
  ...
  async function handleUpgradeCheckout(
    ...
    const result = await onStartCheckout(checkoutPlan);
    if (result.outcome === "redirect") {
      setFeedback({
        tone: "success",
        message: result.message ?? "Upgrade checkout started.",
      });
      return;
    }
    if (result.outcome === "manual_upgrade") {
      // Stripe unavailable: persist a real plan-change request instead of a fake success.
      await submitPlanChangeRequest(requestedPlan, "upgrade");
      return;
    }
    setFeedback({
      tone: "error",
      message: result.message || "Could not start checkout. Please try again.",
    });
  }

  async function handlePlanAction(publicId: PublicPlanId) {
    const plan = getPublicPlan(publicId);
    const rank = getPublicPlanRank(plan.id);
    if (plan.id === currentPublicId || isBusy) {
      return;
    }
    ...
    try {
      if (rank > currentRank) {
        if (!plan.checkoutPlan) {
          await submitPlanChangeRequest(plan.id, "upgrade");
          return;
        }
        await handleUpgradeCheckout(plan.checkoutPlan, plan.id);
        return;
      }

      await submitPlanChangeRequest(plan.id, "downgrade");
    } catch (error) {
      setFeedback({
        tone: "error",
        message: getApiErrorMessage(error, "Could not send plan change request. Please try again."),
      });
    } finally {
  ...
  async function handleHeroCta() {
    ...
    await handlePlanAction(hero.target.id);
  }
      ...
      <div className="space-y-5" data-testid="admin-payments-billing-page">
        {feedback ? (
          <p
            ...
            data-testid="admin-payments-action-feedback"
        ...
        >
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-col items-start gap-2 sm:items-end">
              <div
                className="hidden h-16 w-28 rounded-xl border border-emerald-200 bg-gradient-to-br from-emerald-100 to-white shadow-sm sm:block"
                aria-hidden="true"
              >
              ...
              <button
                ...
                disabled={isBusy && !hero.isHighest}
              ...
              >
                ...
                {isBusy && !hero.isHighest ? "Sending request…" : hero.cta}
              </button>
              {!hero.isHighest ? (
                <p className="text-xs text-gray-500">
                  Stripe checkout when billing is enabled; otherwise a request is sent to superadmin.
                </p>
              ) : null}
            </div>
          </div>
        ...
        >
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {PUBLIC_PLANS.map((plan) => {
              ...
              const loading = actionLoadingPlan === plan.id;
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    className={`mt-5 inline-flex h-11 w-full items-center justify-center rounded-xl text-sm font-semibold outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/40 disabled:cursor-not-allowed ${
                      isCurrent
                        ? "border border-emerald-200 bg-emerald-50 text-emerald-800 disabled:opacity-100"
                        : isUpgrade
                          ? plan.recommended
                            ? "bg-emerald-700 text-white hover:bg-emerald-800 disabled:opacity-60"
                            : "border border-emerald-600 bg-white text-emerald-800 hover:bg-emerald-50 disabled:opacity-60"
                          : "border border-gray-300 bg-white text-gray-800 hover:bg-gray-50 disabled:opacity-60"
                    }`}
                    data-testid={`admin-payments-plan-cta-${plan.id}`}
                  >
                    {loading ? "Sending request…" : ctaLabel}
                  </button>
                </article>
              );
            })}
          ...
          <div className="mt-5 flex flex-wrap items-center justify-between gap-2 text-xs text-gray-500">
            ...
            <p className="font-medium text-gray-500">
              Need help choosing? Reach out via your account support channel.
            </p>
          </div>
        </section>
        ...
        >
          ...
          <ul
            className="flex cursor-default flex-wrap gap-x-4 gap-y-1.5 text-xs text-emerald-800/80 select-none"
            aria-label="Payment security notes"
            data-testid="admin-payments-security-badges"
          >
