...
export function getMiniSitePreviewServiceCardPresentation(
  ...
  switch (template) {
    ...
    case "coach":
      return {
        cardClass: `rounded-xl border-l-4 border p-3 shadow-sm ${baseCard}`,
        titleClass: "text-sm font-bold tracking-tight",
        descriptionClass: `mt-1 text-xs leading-snug ${isDark ? "text-slate-300" : "text-slate-600"}`,
        metaClass: `text-xs font-medium ${isDark ? "text-slate-400" : "text-slate-500"}`,
        buttonClass: `mt-3 block w-full px-3 py-2 text-center text-xs font-semibold text-white shadow-sm ${radius}`,
        iconWrapClass: "rounded-lg bg-white/90 p-1.5",
        buttonVariant: "filled",
      };
    case "clean":
      return {
...
export function getThemedServiceCardPresentation(
  ...
  switch (template) {
    ...
    case "coach":
      return {
        cardClass: `rounded-2xl border-l-4 border p-4 shadow-md ${isDark ? baseCard : "border-slate-200/90 bg-white text-slate-900"}`,
        titleClass: "text-base font-bold break-words",
        descriptionClass: `mt-2 text-sm leading-relaxed break-words ${isDark ? "text-slate-300" : "text-slate-600"}`,
        metaClass: `text-sm font-medium ${isDark ? "text-slate-400" : "text-slate-500"}`,
        buttonClass: `mt-4 block w-full px-4 py-3 text-center text-sm font-semibold text-white shadow-md transition hover:brightness-105 ${radius}`,
        iconWrapClass: "rounded-lg bg-white/90 p-2",
        buttonVariant: "filled",
      };
    case "clean":
      return {
...
export function getMiniSiteTemplatePresentation(
  ...
  switch (template) {
    ...
    case "coach":
      return withTemplateDefaults(isDark, {
        layoutClass: "template-coach w-full",
        layoutSpacingClass: "space-y-0",
        heroClass: "border-0 bg-transparent shadow-none ring-0",
        heroLayoutClass: "flex flex-col gap-6 md:grid md:grid-cols-2 md:items-center",
        heroPaddingClass: "",
        ...
        galleryClass: "border-dashed",
        heroTitleClass:
          "whitespace-normal text-4xl font-bold tracking-tight sm:text-5xl md:text-[3.5rem] lg:leading-[1.05]",
        sectionHeadingClass: "whitespace-normal text-2xl font-bold tracking-tight md:text-3xl lg:text-4xl",
        sectionHeadingAccentClass: "hidden",
        showBenefitsStrip: true,
        showTrustStats: true,
        heroAccentClass: "",
        ...
        heroTopBarClass: "hidden",
        primaryButtonClass: `inline-flex w-full items-center justify-center px-8 py-4 text-base font-semibold text-white shadow-lg transition hover:brightness-110 sm:w-auto ${buttonRadius}`,
        secondaryButtonClass: `inline-flex w-full items-center justify-center border-2 px-8 py-4 text-base font-semibold shadow-sm transition hover:shadow-md sm:w-auto ${buttonRadius} ${
          isDark ? "border-slate-500/80 bg-white/5 text-slate-100 hover:bg-white/10" : "border-white/30 bg-white/10 text-white hover:bg-white/20"
        }`,
      });
    case "clean":
    default:
