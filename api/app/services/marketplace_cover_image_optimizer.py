"""Optimize uploaded marketplace cover images to web-friendly WebP variants."""
...
from app.services.marketplace_cover_image_storage import (
    build_marketplace_cover_public_url,
    resolve_marketplace_cover_upload_path,
)
...
class OptimizedMarketplaceCoverImage:
    web_filename: str
    thumb_filename: str
    web_path: Path
    thumb_path: Path
...
def optimize_marketplace_cover_image(
    business_id: uuid.UUID,
    *,
    content: bytes,
    base_id: str | None = None,
) -> OptimizedMarketplaceCoverImage:
    original_size = len(content)
    base = base_id or uuid.uuid4().hex
    web_filename = f"{base}.webp"
    thumb_filename = f"{base}_thumb.webp"
    web_path = resolve_marketplace_cover_upload_path(business_id, web_filename)
    thumb_path = resolve_marketplace_cover_upload_path(business_id, thumb_filename)
    ...
    return OptimizedMarketplaceCoverImage(
        web_filename=web_filename,
        thumb_filename=thumb_filename,
        web_path=web_path,
        thumb_path=thumb_path,
        web_url=build_marketplace_cover_public_url(business_id, web_filename),
        thumbnail_url=build_marketplace_cover_public_url(business_id, thumb_filename),
