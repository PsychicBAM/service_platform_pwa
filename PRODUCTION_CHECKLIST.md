...
**Start here for launch overview:** [PRODUCTION_LAUNCH_CONTROL_CENTER.md](./PRODUCTION_LAUNCH_CONTROL_CENTER.md) — main index, readiness status, go/no-go.

Related: [DEPLOYMENT.md](./DEPLOYMENT.md), [BACKUP_RESTORE.md](./BACKUP_RESTORE.md), [BACKUP_READINESS_REPORT.md](./BACKUP_READINESS_REPORT.md), [MONITORING_READINESS_REPORT.md](./MONITORING_READINESS_REPORT.md), [LEGAL_PRIVACY_READINESS_REPORT.md](./LEGAL_PRIVACY_READINESS_REPORT.md), [VPS_READINESS_REPORT.md](./VPS_READINESS_REPORT.md), [VPS_DEPLOYMENT_RUNBOOK.md](./VPS_DEPLOYMENT_RUNBOOK.md), [VPS_DRY_RUN_DEPLOYMENT_CHECKLIST.md](./VPS_DRY_RUN_DEPLOYMENT_CHECKLIST.md), [PRODUCTION_LAUNCH_CONTROL_CENTER.md](./PRODUCTION_LAUNCH_CONTROL_CENTER.md), [SECURITY_READINESS_REPORT.md](./SECURITY_READINESS_REPORT.md), [SECURITY_CHECKLIST.md](./SECURITY_CHECKLIST.md), [DEPENDENCY_SECURITY_REPORT.md](./DEPENDENCY_SECURITY_REPORT.md)
...
- [ ] **Payments** (Stripe) — checkout session + webhook + admin checkout + result pages (Slices 6–10); `STRIPE_ENABLED=false` by default; local test setup: [STRIPE_TEST_MODE_GUIDE.md](./STRIPE_TEST_MODE_GUIDE.md); **VPS test-mode runbook:** [STRIPE_TEST_MODE_RUNBOOK.md](./STRIPE_TEST_MODE_RUNBOOK.md) — operator-controlled; test keys only; manual superadmin plan changes separate; run `scripts/check_billing_flow.py` before go-live; billing portal/refunds/downgrades not implemented — see [BILLING_READINESS_REPORT.md](./BILLING_READINESS_REPORT.md); no compliance/payment processing guarantee
- [ ] **Email notifications** — foundation + event wiring implemented; dry-run audit: `docker compose exec api python scripts/check_email_notifications.py` (no real emails); **SMTP readiness:** `docker compose exec api python scripts/check_email_readiness.py` (no real send by default); **operator runbook:** [SMTP_OPERATOR_RUNBOOK.md](./SMTP_OPERATOR_RUNBOOK.md) — disabled → dry-run → one explicit live test → production; live SMTP smoke: `send_test_email.py --to …` or `check_email_readiness.py --send-test …` (one explicit recipient only, after VPS `.env` configured); enable live SMTP with `EMAIL_ENABLED=true`, `EMAIL_DRY_RUN=false`, and SMTP vars on VPS; never commit secrets; no deliverability guarantee
